...
/**
 * The class ConnectedPathValidator is a subclass of PathValidator.It overrides the method Validate()
 * of its parent class and checks whether there is a connected path between startPoint and EndPoint.
 * 
 * @author Mohammad Ali
...
public class ConnectionValidator extends PathValidator {
	...
	/**
	 * This method Validate() overrides the method Validate() of parent class PathValidator. It checks
	 * whether a path exist between startPoint and EndPoint.
	 * 
	...
	public boolean validate() {
		 ...
		 * All road items will be saved in roadItems List for further Validation.
		 */
		...
		/*
		 * Below we will check all MapItem of Type Road and will calculate the number of 
		 * their direct neighbours by calling a helper method NumberOfNeighbours().
		 */
	...
	/**
	 * NumberOfNeighbours is a helper method that gets a road object and count its nwighbours. For a Map
	 * with one entry and exit point and only one path between entry and exit.Every road object must has 
	 * exactly two neighbours unless it is Entry or Exit Point in which case It must have only one connected neighbour.
	 * 
	...
	private int numberOfNeighbours(Road road) {
		...
		/*
		 * Check the neighbours of the given mapPoints and return the one that
		 * is part of path. Limitation: Only work on a single Path.If multiple
		 * paths have to be supported we then the method must return a list of
		 * MapPoints rather than a single MapPoint.
		 */
